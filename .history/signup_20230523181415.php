...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['image'])) {
        ...
        if (in_array($img_extension, $extensions)) {
            ...
            $destination = "./chat_project/images/" . $new_file_name;

            if (move_uploaded_file($temp_name, $destination)) {
                $status = "Active now";
                $random_id = rand(time(), 10000000);
                $sql = "INSERT INTO users_table (user_id, name, email, dob, password, image, status) VALUES ('$random_id', '$name', '$email', '$dob', '$password', '$new_file_name', '$status')";
